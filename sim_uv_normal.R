...
set.seed(15)
dir.create("simdata")
for (N in c(50, 100, 250, 1000, 2500)) {
    mu_truth = c(-4, 0, 5)
...
save(res, file = "simres/uv_normal_qb.Rda")

set.seed(15)
# Dirichlet process mixture model
res = matrix(0, steps - burnins, 5)
alpha = c(0.71, 0.69, 0.67, 0.63, 0.61)
i = 1
for(N in c(50, 100, 250, 1000, 2500)) {
    y = c(unlist(read.table(paste("simdata/data_uv_normal_N_", N, ".txt", sep = ""))))
    res[, i] = runQBSB_slice_efficient_UvGaussian(
...
save(res, file = "simres/uv_normal_dp.Rda")


set.seed(15)
# Pitman--Yor process mixture model
res = matrix(0, steps - burnins, 5)
alpha = c(0.48, 0.46, 0.38, 0.29, 0.29)
d = c(0.09, 0.09, 0.10, 0.11, 0.10)
i = 1
for(N in c(50, 100, 250, 1000, 2500)) {
    y = c(unlist(read.table(paste("simdata/data_uv_normal_N_", N, ".txt", sep = ""))))
    res[, i] = runQBSB_slice_efficient_UvGaussian(
...
save(res, file = "simres/uv_normal_py.Rda")
...
for N in [50, 100, 250, 1000, 2500]
    y = vec(readdlm("simdata\\data_uv_normal_N_" * string(N) * ".txt"))
    options = B.options("Normal", "MFM", y, n_total, n_burn = n_burn, t_max = t_max)
    res = B.run_sampler(options)
    writedlm("simres\\uv_normal_mfm_Sim_N_" * string(N) * ".txt", res.t)
end
...
for(N in c(50, 100, 250, 1000, 2500)) {
     nT <- unlist(read.table(
        paste("simres/uv_normal_mfm_Sim_N_", N, ".txt", sep = "")
        ))[-(1:20000)]
...
postT_uv_normal = data.frame(
    Model = factor(
        rep(c("DP Mixture", "PY Mixture", "QB Mixture", "MFM"),
        each = length(allT) * 5),
        levels = c("DP Mixture", "PY Mixture", "QB Mixture", "MFM")),
    Pr = postT,
    t = allT,
    n = factor(
        rep(c("n=50", "n=100", "n=250", "n=1000", "n=2500"),
        each = length(allT)),
        levels = c("n=50", "n=100", "n=250", "n=1000", "n=2500")) 
)
...
ggsave("postT_uv_normal.png", width = 9, height = 5, units = "in")