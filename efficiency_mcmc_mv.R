...
for (k in 1:5) {
    ...
    write.table(y, paste("simdata/data_mv_normal_250_batch_", k, ".txt", sep = ""),
        row.names = FALSE, col.names = FALSE)
}


source("qbsb_slice_multivariate_gau.R")
kappa = 0.5
...
for(i in 1:5) {
    y = as.matrix(read.table(paste("simdata/data_mv_normal_250_batch_", i, ".txt", sep = "")))
    res[, i] = runQBSB_slice_efficient_MvGaussian(
        y, xi, steps, burnins, p_b = 0.9, eps = 1 / nrow(y) ^ 2.1)
}
save(res, file = "simres/mv_normal_qb_1000_5batch.Rda")
...
for i in [1, 2, 3, 4, 5]
    y = vec(readdlm("simdata\\data_mv_normal_250_batch_" * string(i) * ".txt"))
    options = B.options("Normal", "MFM", y, n_total, n_burn = n_burn, t_max = t_max)
    res = B.run_sampler(options)
    writedlm("simres\\mv_normal_mfm_250_batch_" * string(i) * ".txt", res.t)
end
...
#### input results
load("simres/mv_normal_qb_1000_5batch.Rda") # load QB results

### load MFM results
res_mfm <- c()
for(i in 1:5) {
     res_mfm <- cbind(res_mfm, unlist(read.table(paste(
        "simres/mv_normal_mfm_250_batch_", i, ".txt", sep = "")))[-(1:5000)])    
}
...
ggsave("trace_mv_normal.png", width = width, height = height, units = units)
...
lag.max = 40
thinned_idx = seq(25, nrow(res), by = 25)
...
ggsave("acf_mv_normal.png", width = width, height = height, units = units)